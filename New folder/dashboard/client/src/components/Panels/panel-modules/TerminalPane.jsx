  ...
  useEffect(() => {
    ...
    if (!termRef.current && !socketRef.current) {
      ...
      socketRef.current = io(`http://${host}:1888/pty`, {
        ...
        debug: false, // Disable socket.io debug logs
      })
        ...
        if (termRef.current) {
          ...
          try {
            ...
            errorHandler.showError(
              `Failed to set up terminal event handlers: ${e.message}`,
              ErrorType.UI,
              ErrorSeverity.LOW,
              {
      ...
      socketRef.current.on('connect_error', (error) => {
        ...
        errorHandler.showError(
          `Terminal connection error: ${error.message || 'Failed to connect to terminal service'}`,
          ErrorType.SOCKET,
      ...
      socketRef.current.on('error', (error) => {
        ...
        errorHandler.showError(
          `Terminal error: ${error.message || 'Unknown socket error'}`,
          ErrorType.SOCKET, 
          ErrorSeverity.MEDIUM,
          {
            componentName: 'TerminalPane',
      ...
      socketRef.current.on('pty_error', (data) => {
        ...
        errorHandler.showError(
          `Terminal session error: ${data.message || 'Terminal session ended unexpectedly'}`,
          ErrorType.SOCKET,
          ErrorSeverity.MEDIUM,
          {
            componentName: 'TerminalPane',
            action: 'ptyOperation',
            location: 'TerminalSession',
            metadata: {
      ...
      termRef.current.onData((data) => {
        if (socketRef.current && socketRef.current.connected) {
          try {
            ...
            errorHandler.showError(
              `Terminal input error: ${error.message || 'Failed to send input to terminal'}`,
              ErrorType.SOCKET,
              ErrorSeverity.LOW,
              {
                componentName: 'TerminalPane',
      ...
      socketRef.current.on('connect', () => {
        ...
        setTimeout(() => {
          if (termRef.current && fitAddonRef.current) {
            ...
            // Use initialDimensions if available, otherwise use default or get from fitAddon
            let dims;
            if (termRef.current.initialDimensions) {
              dims = termRef.current.initialDimensions;
            } else {
              const proposedDims = fitAddonRef.current.proposeDimensions();
              dims = {
                cols: proposedDims ? proposedDims.cols : 80,
                rows: proposedDims ? proposedDims.rows : 24
              };
            }
