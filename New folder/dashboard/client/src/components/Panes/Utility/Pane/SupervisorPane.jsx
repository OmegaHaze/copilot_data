...
export default function SupervisorPane({
  ...
  logo,
  moduleData,
  staticIdentifier = 'SupervisorPane'  // Add static identifier with default
}) {
  ...
  // Use moduleType and other data from moduleData
  const moduleType = moduleData?.module_type || 'SYSTEM'; // fallback if not explicitly set
  const paneIdentifier = moduleData?.staticIdentifier || staticIdentifier; // Use provided staticIdentifier
  ...
  useEffect(() => {
    ...
    if (socket && typeof socket.emit === 'function') {
      ...
      socket.emit('getStatus', { name: 'supervisor', moduleType: 'SYSTEM' });
    }
  ...
  return (
    <div className="bg-black rounded border border-green-600 shadow-inner overflow-hidden w-full h-full flex flex-col">
      <PaneHeader 
        ...
        staticIdentifier={paneIdentifier} 
      />
      <div
        ref={terminalRef}
