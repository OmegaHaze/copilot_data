...
async def update_grid_layout(layout: Any = Body(...), user=Depends(get_current_user)):
    """Update the grid layout for a user session
    ...
    try:
        with Session(engine) as db:
            ...
            is_breakpoint_layout = (
                isinstance(layout, dict) and 
                any(bp in layout for bp in ['lg', 'md', 'sm', 'xs', 'xxs'])
            )
            ...
            if is_breakpoint_layout:
                ...
                processed_layout = array_to_dict(layout)
        ...
        logger.error(f"Error updating grid layout: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to update grid layout: {str(e)}")
...
async def delete_grid_layout(user=Depends(get_current_user)):
    """Reset the grid layout to empty state"""
    user_id = user.id
    ...
    try:
        with Session(engine) as db:
            ...
            return {"message": "Grid layout reset successfully"}
    except Exception as e:
        logger.error(f"Error deleting grid layout: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to reset grid layout: {str(e)}")
