...
import { saveLayoutToSession } from './Loader/LayoutManager'

export default function PaneHeader({ name, status: propStatus, logo, moduleType = 'system', _gridId }) {
            ...
            try {
              ...
              Object.keys(updatedLayout).forEach(bp => {
                if (Array.isArray(updatedLayout[bp])) {
                  updatedLayout[bp] = updatedLayout[bp].filter(item => item.i !== _gridId)
                }
              })
