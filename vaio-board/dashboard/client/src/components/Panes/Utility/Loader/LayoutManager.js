...
export async function resolveLayout({ sessionData, items }) {
  ...
  if (fromSession && Object.keys(fromSession).length > 0) {
    ...
    // Also update localStorage to match session (single source of truth)
    saveLayoutToLocal(fromSession);
    return fromSession;
  }
  
  // ONLY if session is empty, try localStorage as a fallback
  const fromStorage = getLayoutFromStorage();
  if (fromStorage && Object.keys(fromStorage).length > 0) {
    console.log('ℹ️ Using layout from localStorage (session was empty)');
    
    // Sync this back to session
    await saveLayoutToSession(fromStorage);
    return fromStorage;
  }
  ...
  // Save this empty layout to both localStorage and the database
  saveLayoutToLocal(emptyLayout);
