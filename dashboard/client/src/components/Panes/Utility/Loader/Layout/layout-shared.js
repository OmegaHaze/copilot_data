...
Object.values(cleaned).forEach(bp => {
  bp.forEach(item => {
    ...
    if (parts.length === 3) {
      ...
      paneIdMap.set(item.i, { moduleType, staticIdentifier, instanceId });
    } else {
      console.warn(`Found item with invalid ID format: ${item.i}. Expected MODULETYPE-STATICID-INSTANCEID`);
    }
  });
});
