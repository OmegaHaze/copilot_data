      ...
      {moduleInitData && (
        <div className="mt-4 glass-notification border border-green-600/20 rounded-md p-4 debug-border-glow">
          ...
          <div className="mb-2">
            <button
              ...
              onClick={async (e) => {
                try {
                  ...
                  const { fetchModules } = await import('../../../Panes/Utility/Loader/Module/module-api');
            ...
            <button
              className="glass-notification border border-amber-700/40 rounded px-3 py-1 text-xs debug-item-hover shadow-glow-sm text-amber-300"
              onClick={async (e) => {
                try {
                  // Visual feedback
                  const button = e.target;
                  ...
                  console.log('Manually syncing active modules with backend...');
                  
                  // Import the module refreshing function with sync flag
                  const { refreshModuleData } = await import('../../../Panes/Utility/Loader/Module/module-index');
                  
                  // Refresh with sync=true to force module synchronization
                  const result = await refreshModuleData(true);
                  console.log('Module sync and refresh result:', result);
                  
                  const registry = (await import('../../../Panes/Utility/Loader/Component/component-registry')).default;
                  
                  // Get the updated data from the registry
                  const verifyData = registry.getModuleData();
                  ...
                  button.innerText = result.success ? "Synced!" : "Sync Failed";
