...
export default function DebugOverlay() {
  ...
  // Define moduleDataRefreshInterval to avoid reference errors
  let moduleDataRefreshInterval;
  ...
  useEffect(() => {
    ...
    const handleToggleEvent = throttle((event) => {
  ...
  // Check API endpoints to diagnose connection issues
  const checkModuleAPI = async () => {
  ...
  useEffect(() => {
    ...
    // Prevent endless loop by ensuring supervisorData is checked before closing overlay
    if (supervisorActive && !supervisorData && !isOpen) {
  ...
  // Force re-fetch session data and module data when debug overlay is opened
  useEffect(() => {
    if (isOpen) {
      fetchSessionData();
      ...
      // Fetch fresh module initialization data
      if (window.debugRegistry && window.debugRegistry.getModuleData) {
        const freshModuleData = window.debugRegistry.getModuleData();
        console.log('Debug overlay opened - refreshing module data:', freshModuleData);
        setModuleInitData(freshModuleData);
      }
      
      // Only refresh module data once when the panel is opened - no automatic refresh
      // Manual refresh is still available via the "Force Refresh Module Data" button
      // This eliminates the continuous API calls
      
      return () => {
        clearInterval(moduleDataRefreshInterval);
        if (buttonRef.current) {
  ...
  useEffect(() => {
    const moduleDataUpdateHandler = (event) => {
      if (event.detail?.moduleData && isOpen) {
        console.log('📊 Module data update event received:', event.detail);
        setModuleInitData(event.detail.moduleData);
      }
    };
    
    window.addEventListener('vaio:module-data-updated', moduleDataUpdateHandler);
    
    return () => {
      window.removeEventListener('vaio:module-data-updated', moduleDataUpdateHandler);
    };
  }, [isOpen]);

  // Throttle the analyzeGridPaneMapping function to limit excessive logging
  const throttledAnalyzeGridPaneMapping = throttle(() => {
    const results = analyzeGridPaneMapping();
    setGridPaneMap({ ...results });

    if (window.debugRegistry && window.debugRegistry.getModuleData) {
      const freshModuleData = window.debugRegistry.getModuleData();
      console.log('Refreshing module init data:', freshModuleData);
      setModuleInitData(freshModuleData);
    }
        ...
        <div className="flex-1 overflow-auto p-4">
          ...
          {activeTab === 'panes' && (
            <DebugOverlayPanes
              ...
              analyzeGridPaneMapping={throttledAnalyzeGridPaneMapping}
            />
          )}
