...
export function useGpuData(setChartData, setCurrentData, setErrorState) {
  ...
  const loadGpuLog = async () => {
    try {
      ...
      // Don't generate fallback data - make error visible
      setError(`Failed to fetch GPU data: ${err.message}`);
      
      // Clear data to make the failure obvious
      dataBuffer.current = [];
      setChartData([]);
      
      // Update current data to show error state
      setCurrentData(prev => ({
        ...prev,
        gpuUtil: 0,
        memUtil: 0,
