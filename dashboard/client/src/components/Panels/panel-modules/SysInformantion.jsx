      ...
      try {
        ...
        if (response.data) {
          ...
          */
          ...
          if (cpuModel) {
            // We expect the backend to provide properly formatted CPU model string
            // If it's incorrect, we'll leave it as-is to make the issue visible
            if (cpuModel.includes("model name:")) {
              console.error("[SysInformation] Backend returned uncleaned CPU model string");
            }
          } else {
            console.error("[SysInformation] Missing CPU model information");
          }
