  ...
  useEffect(() => {
    if (metricsData?.cpu) {
      ...
      const newPoint = {
        ...
        temperature: cpu_temperature, // We won't provide a fallback - let the value come through as-is
        cpuModel: cpu_model || "",
      ...
      setCurrentData(prev => ({
        ...
        cpu: cpu_usage || 0,
        memory: memory_usage || 0,
        temperature: cpu_temperature, // Pass through as-is
        cpuModel: cpu_model || "",
        cpuCores: cpu_cores || 0,
        totalMemoryGB: total_memory_gb || 0
      }));
    ...
    try {
      ...
      if (Array.isArray(cpuHistory) && cpuHistory.length > 0) {
        ...
        const processedData = cpuHistory.map(entry => {
          ...
          // No need for temperature debug logging
          ...
          return {
            ...
            // Pass through temperature data directly without filtering
            temperature: data.cpu_temperature,
            cpuModel: entry.cpu_model || data.cpu_model || systemInfo.cpu_model || "",
