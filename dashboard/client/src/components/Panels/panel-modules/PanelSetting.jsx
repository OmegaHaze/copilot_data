...
export default function PanelSetting({ 
  ...
  const handleResetDb = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/modules/reset-db', {
        method: 'POST'
      });
      ...
      if (json.success) {
        ...
        notify('Failed to reset database: ' + json.error, 'error');
      }
    } catch (err) {
  ...
  const handleDeleteDb = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/modules/clear-db', {
        method: 'DELETE'
      });
      ...
      if (json.success) {
        ...
        console.error('[Frontend] DB Clear Failed:', json.error);
        notify('Failed to clear database: ' + json.error, 'error');
      }
    } catch (err) {
