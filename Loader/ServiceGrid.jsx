...
export default function ServiceGrid({
  ...
  useEffect(() => {
    // Safely import the component registry
    const getRegistry = async () => {
      try {
          ...
          const attemptInitialization = async () => {
            try {
              ...
              if (!window.reloadComponentByName) {
                window.reloadComponentByName = async (componentName) => {
                  try {
                    ...
                    const result = await componentRegistry.reloadComponent(componentName);
              ...
              if (retryCount < maxRetries) {
                ...
                setInitialized(true); // Allow rendering to proceed with default components
              }
            }
          };
        ...
        console.error('Failed to import component registry:', err);
        showError(
          `Failed to import component registry: ${err.message}`,
          ErrorType.SYSTEM,
          ErrorSeverity.HIGH
        );
        setInitialized(true); // Proceed with defaults
      }
    };
  ...
  const addPane = useCallback((item, type, panes) => {
    ...
    console.log(`ServiceGrid: Adding pane for ${moduleKey}`, { 
      hasComponent: !!Component, 
      componentType: Component ? typeof Component : 'undefined',
      props
    });
  ...
  // List of components to render - memoized to prevent unnecessary renders
  const renderedComponents = useMemo(() => {
    // Only render if we have valid layouts and initialization is complete
    if (!hasAnyLayoutsItems || !initialized) return [];
    return renderAll();
  }, [hasAnyLayoutsItems, renderAll, initialized]);
  
  // Loading state component
  if (!initialized) {
    return (
      <div className="p-6 text-center rounded-lg border border-green-800/30 bg-black/30">
        <h3 className="text-lg font-mono text-green-500 mb-4">Initializing Grid System</h3>
